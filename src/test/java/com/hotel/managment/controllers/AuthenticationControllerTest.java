...
public class AuthenticationControllerTest {
  ...
  @Autowired private TokenTypeRepository tokenTypeRepository;
  ...
  public void setup() {
    ...
    TokenType tokenType1 = new TokenType(TokenType.TokenTypeEnum.ACCESS);
    TokenType tokenType2 = new TokenType(TokenType.TokenTypeEnum.RESET_PASSWORD);
    TokenType tokenType3 = new TokenType(TokenType.TokenTypeEnum.EMAIL_CONFIRMATION);
  ...
  public void registerWithInvalidEmail() throws Exception {
    ...
    String responseJson = result.getResponse().getContentAsString();
    assertEquals(
        "{\"status\":\"fail\",\"data\":[\"Email must be a valid email address\"]}", responseJson);
  }
  ...
  public void authenticate() throws Exception {
    AuthenticationRequest authenticationRequest =
        AuthenticationRequest.builder().email("test@example.com").password(password).build();
  ...
  public void authenticateWithInvalidEmail() throws Exception {
    AuthenticationRequest authenticationRequest =
        AuthenticationRequest.builder().email("test2example.com").password(password).build();
    MvcResult result =
        mockMvc
            .perform(
                post("/api/v1/auth/authenticate")
  ...
  public void authenticateWithInvalidPassword() throws Exception {
    AuthenticationRequest authenticationRequest =
        AuthenticationRequest.builder().email("test@example.com").password("1234").build();
    MvcResult result =
        mockMvc
            .perform(
                post("/api/v1/auth/authenticate")
  ...
  public void authenticateNonExistingUser() throws Exception {
    AuthenticationRequest authenticationRequest =
        AuthenticationRequest.builder().email("test3@example.com").password(password).build();
    MvcResult result =
        mockMvc
            .perform(
                post("/api/v1/auth/authenticate")
  ...
  public void confirmEmail() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/auth/email-confirmation/" + testToken))
        .andExpect(status().isOk())
        .andReturn();
  }
  ...
  public void passwordReset() throws Exception {
    PasswordResetRequest passwordResetRequest =
        PasswordResetRequest.builder().email("test@example.com").build();
    mockMvc
        .perform(
  ...
  public void passwordResetConfirmNotValidPass() throws Exception {
    String testToken = "test_token";
    PasswordResetConfirmedRequest passwordResetConfirmedRequest =
        PasswordResetConfirmedRequest.builder().password("1234").build();
    MvcResult result =
        mockMvc
            .perform(
                post("/api/v1/auth/password-reset-confirm/" + testToken)
                    .contentType(MediaType.APPLICATION_JSON)
